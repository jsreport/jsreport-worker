...
module.exports = (options = {}) => {
  ...
  app.use(async ctx => {
    ...
    try {
      ...
      let inputRequest = Buffer.from(ctx.request.body.payload, 'hex')
      inputRequest = processor.reporter.serializer.deserialize(inputRequest)

      const processorResult = await processor.execute(inputRequest)

      const resultBufWithTypes = processor.reporter.serializer.serialize(processorResult)

      ctx.body = {
        payload: resultBufWithTypes.toString('hex')
      }
    } catch (e) {
      console.error(e)
      ctx.status = 400
