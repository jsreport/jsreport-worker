...
module.exports = (options = {}) => {
  ...
  async function executeRecipe ({ req, res }) {
    ...
    if (req.template.recipe === 'xlsx') {
      ...
      // base64 to file paths that exists, in order fpr recipe to do its work normally
  ...
  async function executeScriptManager (uuid, { inputs, options }) {
    ...
    if (inputs.templatingEngines.tempDirectory) {
      inputs.templatingEngines.tempDirectory = reporter.options.tempAutoCleanupDirectory
    }

    if (inputs.templatingEngines.tempCoreDirectory) {
      inputs.templatingEngines.tempCoreDirectory = reporter.options.tempAutoCleanupDirectory
    }
    ...
    if (inputs.response && inputs.response.content) {
      inputs.response.content = Buffer.from(inputs.response.content, 'base64')
    }

    if (inputs.pdfContent != null && inputs.operations != null) {
      options.callback = (operationParams, cb) => {
        inputs.request.uuid = uuid
        return pdfUtilsRenderCallback(reporter, inputs.request, operationParams, cb)
      }
    } else if (inputs.script != null && inputs.method != null) {
      options.callback = (spec, cb) => {
        if (spec.action === 'render') {
          spec.data.req = spec.data.req == null ? {} : spec.data.req
    ...
    if (result.response && result.response.content) {
      result.response.content = (
        Buffer.isBuffer(result.response.content) ? result.response.content : Buffer.from(result.response.content)
      ).toString('base64')
    }
  ...
  reporter.render = async (req, parentReq) => {
    ...
    if (parentReq) {
      if (req.uuid) {
        ...
        uuid = req.uuid
        delete req.uuid
      } else if (parentReq.uuid) {
        // request from pdf-utils
        uuid = parentReq.uuid
