...
module.exports = (options = {}) => {
  ...
  const docxRenderCallback = require('jsreport-docx/lib/scriptCallbackRender')
  const pptxRenderCallback = require('jsreport-pptx/lib/scriptCallbackRender')
  const proxyHandle = require('jsreport-scripts/lib/jsreportProxy').handle

  const proxyHandlers = [
    require('jsreport-scripts/lib/defaultProxyHandlers'),
    require('jsreport-pdf-utils/lib/proxyPdfUtilsHandlers')
  ]
  ...
  async function executeScriptManager (uuid, { inputs, options, req }) {
    ...
    if (inputs.pdfContent != null && inputs.operations != null) {
      options.callback = (operationParams) => {
        req.uuid = uuid

        return new Promise((resolve, reject) => {
          pdfUtilsRenderCallback(reporter, req, operationParams, (err, result) => {
            if (err) {
              return reject(err)
            }
      ...
      options.callback = (spec) => {
        req.uuid = uuid

        return new Promise((resolve, reject) => {
          proxyHandle(reporter, {
    ...
    } else if (inputs.docxTemplateContent != null) {
      options.callback = (operationParams) => {
        req.uuid = uuid

        return new Promise((resolve, reject) => {
          docxRenderCallback(reporter, req, operationParams, (err, result) => {
            if (err) {
              return reject(err)
            }
    ...
    } else if (inputs.pptxTemplateContent != null) {
      options.callback = (operationParams) => {
        req.uuid = uuid

        return new Promise((resolve, reject) => {
          pptxRenderCallback(reporter, req, operationParams, (err, result) => {
            if (err) {
              return reject(err)
            }
    ...
    if (typeof options.callback === 'function') {
      options.callback = createCallbackWithLock(options.callback)
    }
    ...
    try {
      ...
      inputs.timeout = optionsToUse.timeout

      result = await scriptManager.executeAsync(inputs, optionsToUse)
    } catch (e) {
  ...
  reporter.executeScript = async (inputs, options, req) => {
    ...
    return executeScriptManager(uuid, { inputs, options, req })
  }
